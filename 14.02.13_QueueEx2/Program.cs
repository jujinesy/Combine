...
namespace _14._02._13_QueueEx2
{
    class Program
    {
        ...
        {
            // create a Bank
            ThreadPools ThomastonBankandTrust = ThreadPools.Instance; ;
            // create a customer
            ...
            while (true)
            {
                while (ThomastonBankandTrust.TellerLine.Count != 0)
                {
                    ...
                    nextInLine = (Works)ThomastonBankandTrust.TellerLine.Dequeue();
                    //ThomastonBankandTrust.ProcessCustomerRequest(nextInLine);
                    //mouse_event((uint)MouseEventFlags.ABSOLUTE | (uint)MouseEventFlags.MOVE, (uint)nextInLine.X, (uint)nextInLine.Y, 0, 0);
                    uint x = (uint)(nextInLine.X * 65535 / System.Windows.Forms.Screen.PrimaryScreen.WorkingArea.Width);
                    uint y = (uint)(nextInLine.Y * 65535 / System.Windows.Forms.Screen.PrimaryScreen.WorkingArea.Height);
                    ...
                    keybd_event((byte)VirtualKeyShort.LCONTROL, 0, (int)KeybdEventFlags.KEYEVENTF_EXTENDEDKEY, 0); //키보드를 누른 상태 나...
                    keybd_event((byte)VirtualKeyShort.KEY_V, 0, (int)KeybdEventFlags.KEYEVENTF_EXTENDEDKEY, 0); //키보드를 누른 상태 나...
                    keybd_event((byte)VirtualKeyShort.KEY_V, 0, (int)KeybdEventFlags.KEYEVENTF_KEYUP, 0);
                    keybd_event((byte)VirtualKeyShort.LCONTROL, 0, (int)KeybdEventFlags.KEYEVENTF_KEYUP, 0);
                    Console.WriteLine("after :" + ThomastonBankandTrust.TellerLine.Count.ToString());
                    Thread.Sleep(3000);
        ...
        public static void myThread()
        {
            while (true)
            {
                ...
                ThreadPools ThomastonBankandTrust = ThreadPools.Instance;
                Works JP = new Works(pi, "문자내용이 이래요 ㅋ");
                // add the customer to the TellerLine
                ThomastonBankandTrust.TellerLine.Enqueue(JP);
