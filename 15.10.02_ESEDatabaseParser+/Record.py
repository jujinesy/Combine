...
class Record :
    ...
    def readFxiedItem(self) :
        ...
        for i in range(1, self.lastFixedItemID+1) :
            ...
            if type == 17 or type == 14 :
                ...
                self.datas[id] = str(fixedItems.setItem(id, parsingOffset, size)).encode("hex")
            else :
        ...
        for parsingVariableItems in parsingVariableItemsList :
            ...
            if type == 10 :
                try :
                    ...
                    self.datas[id] = str(value).encode("hex")
    ...
    def readTaggedDataItem(self) :
        ...
        for parsingTaggedItems in parsingTaggedItemsList :
            ...
            if id >= 256 and (id in self.columns.keys()) and  self.columns[id].getSpaceUsage() <= 16 and self.columns[id].getSpaceUsage() > 0 :
                ...
                self.datas[id] = ("value", str(value).encode("hex"))
    ...
    def isNormalRecord(self, columns) :
        if self.pageKeySize < 0 or self.pageKeySize >= self.pageSize :
            return False
        
        if not self.lastFixedItemID in columns.keys() :
            return False

        if self.lastFixedItemID >= 128 :
            return False
        
        if not self.lastVariableItemID in columns.keys() and self.lastVariableItemID != 127 : 
            return False
        
        if self.firstVariableItemOffset > self.recordSize :
            return False
...
class LongValueRecord(Record) :
    ...
    def readRecordHeader(self) :
        ...
        elif (self.firstRecordFlag == True and self.pageKeySize == 0) : #V
            self.LvPtrNumber = -1 #it exists in TAG0
            parsingOffset += 4
    ...
    def readRecord(self) :
        ...
        if self.firstRecordFlag :
            ...
            value = ""
            if self.recordSize - self.firstFixedItemOffset < 0 :
                value = ""
            else :
